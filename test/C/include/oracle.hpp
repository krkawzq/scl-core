...
// TODO: Add dense.h when Dense C-API is implemented
...
// Type aliases for compatibility with old test framework
using scl_real_t = double;
using scl_index_t = std::int64_t;
...
inline EigenCSR to_eigen_csr(scl_sparse_t mat) {
    std::int64_t rows = scl_sparse_rows(mat);
    std::int64_t cols = scl_sparse_cols(mat);
    std::int64_t nnz = scl_sparse_nnz(mat);
    scl_layout_t layout = scl_sparse_layout(mat);
    
    if (layout != SCL_LAYOUT_CSR) {
        throw std::runtime_error("Expected CSR matrix");
    }
    
    // TODO: Export data using scl_sparse_to_csr when implemented
    // For now, throw error
    throw std::runtime_error("scl_sparse_export not yet implemented in new API");
...
inline EigenCSC to_eigen_csc(scl_sparse_t mat) {
    ...
    // TODO: Export data using scl_sparse_to_csc when implemented
    throw std::runtime_error("scl_sparse_export not yet implemented in new API");
...
/// TODO: Implement when Dense C-API is available
/*
