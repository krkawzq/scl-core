# 并行编译修复工作流 v2.0

一套模块化、可扩展的并行编译错误修复系统。

## 快速开始

```bash
# 1. 分析编译错误并生成任务
./scripts/parallel_fix/analyze.sh build.log -o tasks.json

# 2. 执行并行修复
./scripts/parallel_fix/run.sh tasks.json

# 3. 或使用交互模式（推荐用于复杂任务）
./scripts/parallel_fix/run.sh --interactive tasks.json

# 4. 验证修复结果
make build 2>&1 | tee build_after_fix.log
```

📖 **详细文档**: 
- [交互式模式完整指南](./INTERACTIVE_MODE.md)
- 包含使用场景、技巧、最佳实践

## 工作流架构

```
┌─────────────────────────────────────────────────────────────────┐
│                        主控制器 (run.sh)                          │
├─────────────────────────────────────────────────────────────────┤
│  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐      │
│  │ Agent 1  │   │ Agent 2  │   │ Agent 3  │   │ Agent N  │      │
│  │ 稀疏API  │   │ 模板参数 │   │ 语法错误 │   │   ...    │      │
│  └────┬─────┘   └────┬─────┘   └────┬─────┘   └────┬─────┘      │
│       │              │              │              │            │
│       └──────────────┴──────────────┴──────────────┘            │
│                              │                                   │
│                    ┌─────────▼─────────┐                        │
│                    │   状态文件 (.state)  │                      │
│                    └───────────────────┘                        │
└─────────────────────────────────────────────────────────────────┘
```

## 文件结构

```
scripts/parallel_fix/
├── README.md           # 本文档
├── run.sh              # 主控制器
├── analyze.sh          # 错误分析器
├── lib/
│   ├── common.sh       # 通用函数库
│   ├── state.sh        # 状态管理
│   └── templates.sh    # 模板生成
├── templates/
│   ├── base.txt        # 基础模板
│   ├── sparse_api.txt  # 稀疏矩阵API模板
│   ├── template_param.txt
│   ├── type_cast.txt
│   └── syntax.txt
└── .state/             # 运行时状态 (gitignore)
    ├── tasks.json      # 任务定义
    ├── status          # 简单状态文件
    └── logs/           # 任务日志
```

## 核心改进

### 1. 简化的状态协议

不再使用复杂的 JSON + jq，改用简单的行式文件：

```
# .state/status 格式
TASK_ID:STATUS:TIMESTAMP:MESSAGE
1:completed:2024-12-29T10:30:00:修复完成
2:in_progress:2024-12-29T10:30:05:正在处理
3:pending:::
```

更新状态只需追加一行：
```bash
echo "1:completed:$(date -Is):修复完成" >> .state/status
```

### 2. 模板继承系统

基础模板 + 特化模板，减少重复：

```bash
# 生成完整prompt
generate_prompt() {
    cat templates/base.txt
    cat templates/$TASK_TYPE.txt
    inject_variables "$TASK_ID" "$TARGET_FILES" "$LOCATIONS"
}
```

### 3. 自动错误分析

从编译日志自动生成任务：

```bash
./analyze.sh build.log
# 输出:
# 发现 6 个独立任务:
# - Task 1: sparse_api (alignment.hpp) - 47 errors
# - Task 2: sparse_api (coexpression.hpp) - 23 errors
# ...
```

### 4. 实时监控

内置监控，无需额外命令：

```bash
./run.sh --monitor
# 自动显示实时进度条和状态
```

## 难度级别系统

系统支持三种难度级别，每种级别使用不同的模型和配置：

| 难度 | 适用场景 | 默认模型 | 思考模式 |
|------|----------|----------|----------|
| `easy` | 缺少头文件、简单语法错误 | Haiku | 关闭 |
| `normal` | API迁移、模板参数修复 | Sonnet 4 | 关闭 |
| `hard` | 复杂逻辑重构、跨文件依赖 | Sonnet 4 | 开启 |

### 指定难度

**任务文件中指定**：
```json
{
  "id": "1",
  "type": "syntax",
  "files": ["src/complex.hpp"],
  "difficulty": "hard"
}
```

**命令行全局覆盖**：
```bash
# 强制所有任务使用 hard 模式
./run.sh --difficulty hard tasks.json
```

**自动检测**：
如果不指定难度，系统会根据任务类型自动检测：
- `missing_header`, `simple_syntax` → easy
- `sparse_api`, `template_param`, `type_cast`, `api_binding` → normal
- `syntax`, `complex_refactor`, `cross_file_dependency` → hard

### 查看配置

```bash
./run.sh --show-config
# 显示当前难度级别对应的模型配置
```

## 配置

编辑 `.parallel_fix.conf` 自定义设置：

```bash
# 最大并行任务数
MAX_PARALLEL=8

# 是否在每个任务后验证编译
VERIFY_AFTER_TASK=false

# 失败重试次数
RETRY_COUNT=1

# ============================================================
# 难度级别模型配置
# ============================================================

# Easy: 简单任务
EASY_MODEL="claude-haiku-4-5-20251001"
EASY_THINK=false

# Normal: 常规任务
NORMAL_MODEL="claude-sonnet-4-5-20250929"
NORMAL_THINK=false

# Hard: 复杂任务 - 启用深度思考
HARD_MODEL="claude-opus-4-5-20251101"
HARD_THINK=true

# 默认难度级别
DEFAULT_DIFFICULTY="normal"
```

## 使用示例

### 基本流程

```bash
# 1. 编译并捕获错误
make build 2>&1 | tee build.log

# 2. 分析错误生成任务
./scripts/parallel_fix/analyze.sh build.log -o tasks.json

# 3. 预览任务（干运行）
./scripts/parallel_fix/run.sh --dry-run tasks.json

# 4. 执行修复
./scripts/parallel_fix/run.sh tasks.json

# 5. 验证
make build
```

### 交互式模式 🆕

交互模式会自动发送第一个任务的prompt，然后让你继续与Claude对话：

```bash
# 交互式修复 - 自动开始对话
./scripts/parallel_fix/run.sh --interactive tasks.json

# 交互模式 + hard难度
./scripts/parallel_fix/run.sh -i --difficulty hard tasks.json
```

**工作流程：**
1. 系统加载第一个任务
2. 自动生成并发送初始prompt
3. Claude开始处理并响应
4. **你可以继续输入追问、调整要求、查看更多细节**
5. 完成后手动标记状态（或退出）

**适用场景：**
- 复杂问题需要多轮讨论
- 需要在修复过程中调整策略
- 想要深入理解错误原因
- 需要逐步验证修复方案

### 高级用法

```bash
# 使用 hard 模式处理所有任务
./scripts/parallel_fix/run.sh --difficulty hard tasks.json

# 带监控模式运行
./scripts/parallel_fix/run.sh --monitor tasks.json

# 一键快速修复
./scripts/parallel_fix/quick_fix.sh

# 详细输出模式
./scripts/parallel_fix/run.sh --verbose tasks.json
```

