...
// =============================================================================
// Scalar Implementations (Double Precision Kernel)
// =============================================================================

/// @brief Fast approximation of complementary error function erfc(x).
/// Uses Abramowitz and Stegun rational approximation (precision ~1e-7).
/// Computation is done in double precision for stability.
SCL_FORCE_INLINE double erfc(double x) {
    const double ax = std::abs(x);
    const double t  = 1.0 / (1.0 + 0.5 * ax);
    
    // Horner's method or explicit expansion (as per original code)
...
SCL_FORCE_INLINE double normal_sf(double z) {
    return 0.5 * erfc(z * 0.7071067811865475); // 1/sqrt(2)
}
...
namespace simd {
    ...
    // Import scl::simd namespace for convenience
    namespace s = scl::simd;

    /// @brief SIMD version of fast_erfc.
    /// @tparam D Highway Simd Tag (e.g., scl::simd::Tag)
    ...
    SCL_FORCE_INLINE V erfc(D d, V x) {
        ...
        // Note: For optimal performance, we construct constants on the fly
