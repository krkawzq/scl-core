...
constexpr const T* lower_bound(const T* first, const T* last, const V& target) noexcept {
...
template <TotallyOrdered T, typename V>
    requires std::totally_ordered_with<T, V>
SCL_FORCE_INLINE SCL_HOT
constexpr const T* upper_bound(const T* first, const T* last, const V& target) noexcept {
    SCL_ASSUME(first <= last);
...
SCL_FORCE_INLINE SCL_HOT
constexpr const T* lower_bound(const T* first, const T* last, const V& target, Cmp cmp) noexcept {
    SCL_ASSUME(first <= last);
...
template <Movable T>
SCL_FORCE_INLINE constexpr void insertion_sort(T* first, T* last) noexcept {
...
SCL_FORCE_INLINE constexpr T* median_of_three(T* a, T* b, T* c) noexcept {
    if (*a < *b) {
        if (*b < *c) return b;
        if (*a < *c) return c;
        return a;
...
template <Movable T>
SCL_FORCE_INLINE constexpr T* partition(T* first, T* last, const T& pivot) noexcept {
    while (true) {
...
constexpr void nth_element(T* first, T* nth, T* last) noexcept {
...
template <typename T>
SCL_FORCE_INLINE void fill(T* dst, Size n, const T& value) noexcept {
...
// Fast zero
template <typename T>
SCL_FORCE_INLINE void zero(T* dst, Size n) noexcept {
...
// =============================================================================
// SECTION 4: Reduction Operations (unchecked)
// =============================================================================

// Sum with 4-way unroll
template <typename T>
SCL_FORCE_INLINE T sum(const T* data, Size n) noexcept {
    namespace s = scl::simd;
    using SimdTag = s::SimdTagFor<T>;
    const SimdTag d;
    const Size lanes = s::Lanes(d);
...
// Max element
template <TotallyOrdered T>
SCL_FORCE_INLINE T max(const T* data, Size n) noexcept {
    SCL_ASSUME(n > 0);
...
// Min element
template <TotallyOrdered T>
SCL_FORCE_INLINE T min(const T* data, Size n) noexcept {
    SCL_ASSUME(n > 0);
...
// =============================================================================
// SECTION 5: Heap Operations (unchecked)
// =============================================================================

// Max-heap sift down
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void heap_sift_down(T* data, Size heap_size, Size pos) noexcept {
    while (true) {
        Size largest = pos;
        Size left = 2 * pos + 1;
        Size right = 2 * pos + 2;
...
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void heap_sift_up(T* data, Size pos) noexcept {
    while (pos > 0) {
        Size parent = (pos - 1) / 2;
...
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void make_heap(T* data, Size n) noexcept {
    if (n < 2) return;
...
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void heap_push(T* data, Size n) noexcept {
    if (n > 1) {
...
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void heap_pop(T* data, Size n) noexcept {
    if (n > 1) {
...
// =============================================================================
// SECTION 6: Counting and Searching
// =============================================================================

// Count elements equal to value
template <typename T, typename V>
SCL_FORCE_INLINE constexpr Size count(const T* data, Size n, const V& value) noexcept {
    Size result = 0;
...
template <typename T, typename V>
SCL_FORCE_INLINE constexpr Size find(const T* data, Size n, const V& value) noexcept {
    for (Size i = 0; i < n; ++i) {
...
SCL_FORCE_INLINE constexpr void iota(T* data, Size n, T start = T(0)) noexcept {
    for (Size i = 0; i < n; ++i) {
...
template <Movable T>
SCL_FORCE_INLINE constexpr void reverse(T* data, Size n) noexcept {
    T* left = data;
    T* right = data + n - 1;
...
// =============================================================================
// SECTION 8: Swap and Compare Utilities
// =============================================================================

// Swap two elements
template <Movable T>
SCL_FORCE_INLINE constexpr void swap(T& a, T& b) noexcept {
...
// Min of two values
template <TotallyOrdered T>
SCL_FORCE_INLINE constexpr T min2(const T& a, const T& b) noexcept {
    return (a < b) ? a : b;
}

// Max of two values
template <TotallyOrdered T>
SCL_FORCE_INLINE constexpr T max2(const T& a, const T& b) noexcept {
    return (a > b) ? a : b;
}

// Clamp value to range
template <TotallyOrdered T>
SCL_FORCE_INLINE constexpr T clamp(const T& val, const T& lo, const T& hi) noexcept {
    return (val < lo) ? lo : ((val > hi) ? hi : val);
}

// =============================================================================
// SECTION 9: ArgMax/ArgMin
// =============================================================================

// Find index of maximum element
template <TotallyOrdered T>
SCL_FORCE_INLINE Size argmax(const T* data, Size n) noexcept {
    SCL_ASSUME(n > 0);
...
template <TotallyOrdered T>
SCL_FORCE_INLINE Size argmin(const T* data, Size n) noexcept {
    SCL_ASSUME(n > 0);
...
template <TotallyOrdered T>
SCL_FORCE_INLINE void minmax(const T* data, Size n, T& out_min, T& out_max) noexcept {
    SCL_ASSUME(n > 0);
...
// =============================================================================
// SECTION 10: Min-Heap Operations (for bottom-k selection)
// =============================================================================

// Min-heap sift down
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void min_heap_sift_down(T* data, Size heap_size, Size pos) noexcept {
    while (true) {
        Size smallest = pos;
        Size left = 2 * pos + 1;
        Size right = 2 * pos + 2;
...
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void min_heap_sift_up(T* data, Size pos) noexcept {
    while (pos > 0) {
        Size parent = (pos - 1) / 2;
        if (data[pos] >= data[parent]) break;
...
template <Movable T>
    requires TotallyOrdered<T>
SCL_FORCE_INLINE constexpr void make_min_heap(T* data, Size n) noexcept {
    if (n < 2) return;
...
// =============================================================================
// SECTION 11: Partial Sort and Heap Sort
// =============================================================================

// Heap sort (ascending order)
template <Movable T>
    requires TotallyOrdered<T>
constexpr void heap_sort(T* data, Size n) noexcept {
    if (n < 2) return;
...
template <Movable T>
    requires TotallyOrdered<T>
constexpr void partial_sort(T* data, Size n, Size k) noexcept {
    if (k == 0 || n == 0) return;
    if (k >= n) {
        heap_sort(data, n);
        return;
...
constexpr void partial_sort(T* data, Size n, Size k, Cmp cmp) noexcept {
    if (k == 0 || n == 0) return;
    if (k >= n) k = n;
...
// =============================================================================
// SECTION 12: Sparse Vector Operations
// =============================================================================
...
template <typename T>
SCL_FORCE_INLINE Size sparse_intersection_size(
...
// =============================================================================
// SECTION 13: Range Operations
// =============================================================================

// Check if array is sorted
template <TotallyOrdered T>
SCL_FORCE_INLINE constexpr bool is_sorted(const T* data, Size n) noexcept {
    for (Size i = 1; i < n; ++i) {
...
template <Movable T>
constexpr Size unique(T* data, Size n) noexcept {
    if (n <= 1) return n;
...
template <Movable T>
constexpr void rotate_left(T* data, Size n, Size k) noexcept {
    if (n == 0 || k == 0) return;
    k = k % n;
    if (k == 0) return;
...
constexpr Size partition(T* data, Size n, Pred pred) noexcept {
    Size write = 0;
...
SCL_FORCE_INLINE constexpr void gather(
    const T* SCL_RESTRICT src,
...
template <typename T, typename I>
    requires std::integral<I>
SCL_FORCE_INLINE constexpr void scatter(
    const T* SCL_RESTRICT src,
    const I* SCL_RESTRICT indices,
    T* SCL_RESTRICT dst,
    Size n
