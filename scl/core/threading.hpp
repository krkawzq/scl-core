...
///   - OpenMP (SCL_BACKEND_OPENMP)
///   - Intel TBB (SCL_BACKEND_TBB)
///   - BS::thread_pool (SCL_BACKEND_BS)
///   - Serial execution (SCL_BACKEND_SERIAL)
///
/// Key Features:
...
///   - ThreadLocal: Thread-local storage wrapper
///
...
[[nodiscard]] constexpr auto get_backend() noexcept -> Backend {
    #if defined(SCL_USE_OPENMP)
...
[[nodiscard]] constexpr auto get_backend_name() noexcept -> const char* {
    #if defined(SCL_THREADING_NAME)
        return SCL_THREADING_NAME;
    #else
        return "Unknown";
    #endif
}
...
[[nodiscard]] constexpr auto is_parallel_available() noexcept -> bool {
...
[[nodiscard]] inline auto get_num_threads() noexcept -> Size {
    #if defined(SCL_USE_OPENMP)
...
/// @param num_threads Desired number of threads (0 = use hardware default)
/// @note Some backends may not support dynamic thread count changes
inline void set_num_threads(Size num_threads) noexcept {
    if (num_threads == 0) {
        ...
        // This is a no-op, pool size is fixed at construction
        (void)num_threads;
    #else
        (void)num_threads;
    #endif
...
[[nodiscard]] inline auto get_thread_id() noexcept -> Size {
    #if defined(SCL_USE_OPENMP)
...
[[nodiscard]] inline auto hardware_concurrency() noexcept -> Size {
...
/// @param grain_size Minimum iterations per thread (hint, used by TBB; ignored by OpenMP/BS)
///
/// @note The function is called with indices in [begin, end)
/// @note Order of execution is not guaranteed
/// @note grain_size is only used by the TBB backend; OpenMP uses static scheduling
///
/// Example:
/// @code
...
inline void parallel_for(Index begin, Index end, Func&& func, Size grain_size = DEFAULT_GRAIN_SIZE) {
    if (begin >= end) return;
    ...
    #elif defined(SCL_USE_TBB)
        tbb::parallel_for(
            tbb::blocked_range<Index>(begin, end, grain_size),
...
/// @param func Function to execute for each index
template<typename Func>
inline void parallel_for(Size count, Func&& func) {
...
[[nodiscard]] auto parallel_reduce(
    ...
    if (count < MIN_PARALLEL_SIZE || !is_parallel_available()) {
        ...
        detail::get_thread_pool().detach_blocks(
            begin, end,
            [&map, &reduce, identity, &partial_results, &results_mutex](
...
[[nodiscard]] auto parallel_sum(Index begin, Index end, MapFunc&& map) -> T {
    return parallel_reduce(begin, end, T{0}, std::forward<MapFunc>(map), std::plus<T>{});
}
...
/// @param funcs Functions to execute in parallel
///
/// Example:
/// @code
///   scl::threading::parallel_invoke(
///       [&] { compute_a(data); },
///       [&] { compute_b(data); },
///       [&] { compute_c(data); }
///   );
...
inline void parallel_invoke(Funcs&&... funcs) {
...
// =============================================================================
// SECTION 7: Thread-Local Storage
// =============================================================================
...
public:
    ...
    /// @param init Function that creates a new T for each thread
    ...
    [[nodiscard]] auto get() const -> const_reference {
        const Size idx = get_thread_index();
        ensure_initialized(idx);
        return values_[idx];
    }
...
private:
    ...
    void ensure_initialized(Size idx) const {
...
public:
    ...
    [[nodiscard]] auto combine(T identity, ReduceFunc&& reduce) const -> T {
        T result = identity;
        for (Size i = 0; i < values_.size(); ++i) {
            if (initialized_[i].load(std::memory_order_acquire)) {
...
private:
    [[nodiscard]] static auto get_thread_index() -> Size {
        #if defined(SCL_USE_OPENMP)
            return static_cast<Size>(omp_get_thread_num());
        #elif defined(SCL_USE_TBB)
...
// =============================================================================
// SECTION 7.5: Thread Work Area (Reusable Per-Thread Buffers)
// =============================================================================
...
public:
    ...
    /// @param init Function to initialize work area (called once per thread)
    explicit ThreadWorkArea(initializer_type init)
        : ThreadWorkArea(std::move(init), nullptr) {}
    
    /// @brief Construct with initializer and resetter
    /// @param init Function to initialize work area (called once per thread)
    /// @param reset Function to reset work area (called before each use)
    ...
    void mark_reset_all() noexcept {
        for (Size i = 0; i < num_slots_; ++i) {
            auto expected = State::Ready;
            state_[i].compare_exchange_strong(
                expected, State::NeedsReset,
    ...
    /// @param func Function to apply to each work area
    template<typename Func>
    void for_each(Func&& func) {
        for (Size i = 0; i < num_slots_; ++i) {
            State s = state_[i].load(std::memory_order_acquire);
    ...
    [[nodiscard]] auto combine(R identity, ReduceFunc&& reduce) const -> R {
    ...
    [[nodiscard]] auto num_initialized() const noexcept -> Size {
        Size count = 0;
        for (Size i = 0; i < num_slots_; ++i) {
    ...
    [[nodiscard]] auto num_slots() const noexcept -> Size {
        return num_slots_;
    }
    
    /// @brief Check if all slots are initialized
    [[nodiscard]] auto all_initialized() const noexcept -> bool {
...
private:
    ...
    void ensure_ready(Size idx) const {
...
/// @param buffer_size Size of each thread's buffer
...
/// @endcode
template<typename T>
[[nodiscard]] auto make_work_buffers(Size buffer_size) 
    -> ThreadWorkArea<std::vector<T>> {
    return ThreadWorkArea<std::vector<T>>(
        // Initializer: allocate buffer
        [buffer_size](std::vector<T>& buf) {
...
[[nodiscard]] auto make_fixed_work_buffers()
    -> ThreadWorkArea<std::array<T, N>> {
...
// =============================================================================
// SECTION 8: Synchronization Primitives
// =============================================================================
...
///
/// Ensures the function is executed by only one thread at a time.
template<typename Func>
inline void critical(Func&& func) {
    #if defined(SCL_USE_OPENMP)
        #pragma omp critical
        { func(); }
    #else
...
[[nodiscard]] inline auto in_parallel_region() noexcept -> bool {
    #if defined(SCL_USE_OPENMP)
...
/// @param total_size Total number of iterations
/// @param min_per_thread Minimum iterations per thread
/// @return Recommended grain size
[[nodiscard]] inline auto calculate_grain_size(
    Size total_size,
    Size min_per_thread = 64
) noexcept -> Size {
    const Size num_threads = get_num_threads();
    if (num_threads == 0) return total_size;
...
/// @param element_cost Estimated cost per element (higher = more beneficial)
/// @return true if parallel execution is recommended
[[nodiscard]] inline auto should_parallelize(
    Size size,
    Size element_cost = 1
) noexcept -> bool {
    if (!is_parallel_available()) return false;
