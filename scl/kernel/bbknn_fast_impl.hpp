...
/// @brief Extreme Performance BBKNN
///
...
SCL_FORCE_INLINE void compute_norms_bbknn_custom_fast(
...
/// @brief Ultra-fast norm precomputation (VirtualSparse)
///
/// Optimization: Row-wise SIMD with minimal pointer dereference
template <typename T, bool IsCSR>
    requires VirtualSparseLike<VirtualSparse<T, IsCSR>, IsCSR>
SCL_FORCE_INLINE void compute_norms_bbknn_virtual_fast(
...
SCL_FORCE_INLINE void compute_norms_bbknn_fast(
    ...
    if constexpr (CustomSparseLike<MatrixT, IsCSR>) {
        compute_norms_bbknn_custom_fast(matrix, norms_sq);
    } else if constexpr (VirtualSparseLike<MatrixT, IsCSR>) {
        compute_norms_bbknn_virtual_fast(matrix, norms_sq);
    }
}
