...
/// @brief Batch Balanced K-Nearest Neighbors (Generic CSR-like matrices).
///
...
void bbknn(
    ...
    scl::threading::parallel_for(0, n_chunks, [&](size_t chunk_idx) {
        ...
        for (size_t i = i_start; i < i_end; ++i) {
            ...
            auto idx_i = matrix.row_indices(query_idx);
            auto val_i = matrix.row_values(query_idx);
            Index len_i = matrix.row_length(query_idx);
            ...
            for (Size j = 0; j < N; ++j) {
                ...
                auto idx_j = matrix.row_indices(target_idx);
                auto val_j = matrix.row_values(target_idx);
                Index len_j = matrix.row_length(target_idx);
...
/// @brief BBKNN with automatic batch count detection (Generic CSR-like matrices).
///
